var shopManager = function(router,connection,config){
...
router.get('/shopManager', function(req, res, next) {
	connection.query('select * from elm_shoplist',function(error,results,fields){
		...
		if(req.query.pageNum){
			//显示从第5x条开始的后面5条信息
			connection.query(`select * from elm_shoplist limit ${req.query.pageNum*7},7`,function(error,results,fields){
				obj.listArr = results;
				obj.pageNum = req.query.pageNum;
				res.render('shop/shopManager',obj)
			})
			
		}else{
			//从第0条开始的后面5条，一开始就渲染的。
			connection.query(`select * from elm_shoplist limit 0,7`,function(error,results,fields){
				console.log(results);
				obj.listArr = results;
				obj.pageNum = 0;
				res.render('shop/shopManager',obj)
