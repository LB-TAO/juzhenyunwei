var mokuaiSafe = function(router,connection,config){
	router.get('/mokuaiSafe',function(req,res,next){
		// 用户组权限
		connection.query('select * from elm_authlist',function(error,results,fields){
			...
			if(req.query.pageNum){
				// 从第5x条开始的后面5条信息
				connection.query(`select * from elm_authlist limit ${req.query.pageNum*7},7`,function(error,results,fields){
					obj.listArr = results;
					obj.page.Num = req.query.pageNum;
					res.render('safe/mokuaiSafe.html',obj);
				})
			}else{
				// 一开始没有pageNum的存在执行的逻辑
				connection.query(`select * from elm_authlist limit 0,7`,function(error,results,fields){
					console.log(results);
					obj.listArr = results;
