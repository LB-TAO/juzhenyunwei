var renlianSafe = function(router,connection,config){
	
	
	//店铺列表页面
router.get('/renlianSafe', function(req, res, next) {
	connection.query('select * from elm_renlian',function(error,results,fields){
		var obj = {
			// 要传给html模板中。
		...
		if(req.query.pageNum){
			//显示从第5x条开始的后面5条信息
			connection.query(`select * from elm_renlian limit ${req.query.pageNum*7},7`,function(error,results,fields){
				obj.listArr = results;
				obj.pageNum = req.query.pageNum;
				res.render('safe/renlianSafe',obj)
			})
			
		}else{
			//从第0条开始的后面5条，一开始就渲染的。
			connection.query(`select * from elm_renlian limit 0,7`,function(error,results,fields){
				console.log(results);
				obj.listArr = results;
				obj.pageNum = 0;
				res.render('safe/renlianSafe',obj)
